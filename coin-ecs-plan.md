...
3. **GPU-Driven Object Buffer (Bindless Resources)**
   - Upload the flattened ECS array of structural render properties (transforms, material IDs, and bounding boxes) into a single large GPU Shader Storage Buffer Object (SSBO) asynchronously.
   - Material parameters and Textures are bound using Mega-Textures or Bindless array buffers. 

4. **Compute-Based Visibility / Frustum Culling**
   - A GPU Compute Shader reads the SSBO Bounding Boxes, performs Frustum Culling (and potentially Hierarchical Z-Buffer Occlusion Culling), and populates an active command buffer natively on the GPU, completely eliminating CPU culling bottlenecks.

5. **Multi-Draw Indirect (MDI)**
   - The localized indices correspond directly to multi-draw execution properties. The ModernGLBackend consumes the indirect commands using `glMultiDrawElementsIndirect` mapping back to an all-in-one Global Geometry VBO caching all shape definitions.
...
flowchart TD
    ...
    subgraph GPUStorage [GPU Persisted Buffers]
        SOA -- Async Upload --> SSBO[("Scene SSBO")]
        GeoCache[("Global Unified Index/Vertex VBO")]
    end

    subgraph GPUPipeline [GPU-Driven Execution]
        cam["Camera Data"] --> Compute["Compute Shader: Culling"]
        SSBO --> Compute
...
- **Goal:** Bind the CPU ECS arena to the GPU.
- Define the GPU SSBO layouts mapping directly to the CPU ECS arrays (zero-copy or direct buffer mappings where possible).
- Dispatch a compute shader mapping Frustum Culling against the SSBO Bounding Boxes to pack an output `Indirect Buffer`.

#### Phase 3: Bindless Multi-Draw Execution
- **Goal:** Reduce driver call overhead.
- Aggregate all mesh descriptions into a unified Vertex and Index buffer (`SoModernGLBackend` index).
- Substitute the localized geometry renderer with a single `GL_DRAW_INDIRECT_BUFFER` MDI execution.
...
> - **Hardware Baseline:** We are assuming hardware availability of OpenGL 4.3+ (Compute Shaders & SSBOs) for the Modern path. Is this minimum specification acceptable?
